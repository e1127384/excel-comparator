...
It handles missing records, field-by-field discrepancies, flexible date normalization (supporting text/general formats like `06 Aug 2025` and `10-Mar-2027` without time components), list separator normalization (e.g., `,` vs `;`), and case-sensitivity toggles, exporting all findings neatly into an output Excel report.
...
```bash
go run main.go -f1 file1.xlsx -f2 file2.xlsx -output ./results/audit_diff.xlsx
...
* **`Sheet1 Value`**: The raw or normalized value recorded in the first file.
* **`Sheet2 Value`**: The raw or normalized value recorded in the second file.
